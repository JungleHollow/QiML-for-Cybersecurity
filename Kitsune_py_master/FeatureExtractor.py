...
# Import dependencies
import Kitsune_py_master.netStat as ns
import csv
import numpy as np
...
class FE:
    ...
    def get_next_vector(self):
        ...
        if self.parse_type == "tsv":
            ...
            if row[4] != '':  # IPv4
                ...
                IPtype = 1
            srcproto = row[6] + row[
                8]  # UDP or TCP port: the concatenation of the two port strings will store results in an OR "[tcp|udp]"
            ...
            current_packet = self.scapyin[self.curPacketIndx]
            IPtype = np.nan
            timestamp = current_packet.time
            framelen = len(current_packet)
            if current_packet.haslayer(IP):  # IPv4
                srcIP = current_packet[IP].src
                dstIP = current_packet[IP].dst
                IPtype = 0
            elif current_packet.haslayer(IPv6):  # ipv6
                srcIP = current_packet[IPv6].src
                dstIP = current_packet[IPv6].dst
                IPtype = 1
            else:
                srcIP = ''
                dstIP = ''

            if current_packet.haslayer(TCP):
                srcproto = str(current_packet[TCP].sport)
                dstproto = str(current_packet[TCP].dport)
            elif current_packet.haslayer(UDP):
                srcproto = str(current_packet[UDP].sport)
                dstproto = str(current_packet[UDP].dport)
            else:
                srcproto = ''
                dstproto = ''

            srcMAC = current_packet.src
            dstMAC = current_packet.dst
            if srcproto == '':  # it's a L2/L1 level protocol
                if current_packet.haslayer(ARP):  # is ARP
                    srcproto = 'arp'
                    dstproto = 'arp'
                    srcIP = current_packet[ARP].psrc  # src IP (ARP)
                    dstIP = current_packet[ARP].pdst  # dst IP (ARP)
                    IPtype = 0
                elif current_packet.haslayer(ICMP):  # is ICMP
                    srcproto = 'icmp'
                    dstproto = 'icmp'
                    IPtype = 0
                elif srcIP + srcproto + dstIP + dstproto == '':  # some other protocol
                    srcIP = current_packet.src  # src MAC
                    dstIP = current_packet.dst  # dst MAC
        else:
            return []
    ...
    def pcap2tsv_with_tshark(self):
        ...
        tshark_cmd = '"' + self._tshark + '" -r ' + self.path + ' -T fields ' + fields + ' -E header=y -E occurrence=f > ' + self.path+".tsv"
